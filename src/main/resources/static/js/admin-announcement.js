...
async function openReadStatusModal(a) {
  const msgEl = document.getElementById("readStatusMsg");
  msgEl.textContent = "読み込み中…";
  msgEl.classList.remove("error");
  msgEl.classList.add("is-loading");
  ...
  try {
    ...
    if (!res.ok) {
      msgEl.textContent = await readErrorMessage(res, "既読状況の取得に失敗しました");
    ...
    console.error(e);
    msgEl.textContent = "既読状況の取得に失敗しました";
    msgEl.classList.add("error");
  } finally {
    msgEl.classList.remove("is-loading");
  }
}
